...
{
    public static Texture2D capture(Rect pRect, Camera yourCam)
    {
        var lCamera = yourCam;
        Texture2D lOut;
    ...
    /// </summary>
    /// <returns></returns>
    public static Texture2D captureScreenshot(Camera Cam)
    {
        return capture(new Rect(0f, 0f, Screen.width, Screen.height),Cam);
    }
    ...
    /// <returns></returns>
    public static byte[] captureScreenshot(string pFileName,Camera camera)
    {
        var lScreenshot = captureScreenshot(camera);
        try
        {
            ...
            var bytes = lScreenshot.EncodeToPNG();
            lWriter.Write(bytes);
            return bytes;
        }
        finally
